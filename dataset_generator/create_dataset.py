...
def append_landmarks(left_hand_landmarks, right_hand_landmarks, pose_landmarks, n_frame, frame_list):
    if left_hand_landmarks is not None:
        for id, lm in enumerate(left_hand_landmarks.landmark):
            ...
            frame_list[f'X{id}{n_frame*198}'] = x
            frame_list[f'Y{id}{n_frame*198}'] = y
            frame_list[f'Z{id}{n_frame*198}'] = z
    ...
    # for i in range(66):
    #     frame[i*3] -= frame[159]
    #     frame[i*3 + 1] -= frame[160]
    #     frame[i*3 + 2] -= frame[161]

    # #Normalize the points with respect to the shoulder landmarks (index 156)
    # for i in range(66):
    #     frame[i*3] /= frame[156]
    #     frame[i*3 + 1] /= frame[157]
    #     frame[i*3 + 2] /= frame[158]
